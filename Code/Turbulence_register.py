...
N = int(sys.argv[1])
M = int(sys.argv[2])
Nshift = int(sys.argv[3])
ii = int(sys.argv[4])
jj = int(sys.argv[5])
stop_epoch = int(sys.argv[6])
lr = float(sys.argv[7])
save_name = sys.argv[9]

sidelobe = 5
for frame_num in range(1, Nshift*Nshift+1):
    wigner_name = sys.argv[8]
    Phantom_stack = tiff3Dread(wigner_name+'test_No_'+str(ii+1)+'_'+str(jj+1)+'.tif')
    ...
    Phantom_fix = torch.tensor(Phantom_stack[:, :, frame_num])
    ...
    criterion = torch.nn.MSELoss(reduction='sum').to(param.device)# 交叉熵
    optimizer = torch.optim.Adam([Model.ShiftMap], lr=lr)

    ##

    ProjectName = save_name + 'Wigner_'+str(ii+1)+'_'+str(jj+1)+'/' + str(frame_num) + '/'
    ...
    for epoch in range(0, stop_epoch):
        ...
        warped_Phantom, ShiftMap_interp = Model(Phantom_fix_gpu)

        # 计算损失函数
        loss = criterion(torch.squeeze(warped_Phantom[:, :, sidelobe:-1 - sidelobe, sidelobe:-1 - sidelobe]),
                         torch.squeeze(Phantom_gpu[sidelobe:-1 - sidelobe, sidelobe:-1 - sidelobe]))
        # loss = criterion(torch.squeeze(warped_Phantom), torch.squeeze(Phantom_gpu))
        ...
        sys.stdout.write("[Train {}|{}||{}] [Epoch {}/{}] [loss:{:.8f}] time {:.3f}\n"
                         .format(ii, jj, frame_num, epoch + 1, stop_epoch, loss.item(), time() - Time_1))
        sys.stdout.flush()
