function [w] = solve_image_fast_gyd(v, beta, alpha)
...
%   fprintf('Reusing lookup table for beta %.3g and alpha %.3g\n', beta, alpha);
  ...
  if (exist('pointOp') == 3) 
    ...
    w = interp1(gpuArray(single(xx')), lookup_v(ind,:)', v(:), 'linear', 'extrap');
    w = reshape(w, size(v,1), size(v,2));
     ...
     if (exist('pointOp') == 3) 
       ...
       w = interp1(gpuArray(single(xx')), lookup_v(end,:)', v(:), 'linear', 'extrap');
       w = reshape(w, size(v,1), size(v,2));
     end
